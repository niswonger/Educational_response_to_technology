...
dt.wage_growth <- dt.census[WKSWORK2 >= 5 & EMPSTAT == 1 & AGE %in% 25:55 ,list(wage_growth = (.SD[rank == 5, mean(INCWAGE)]-.SD[rank == 1, mean(INCWAGE)])/.SD[rank == 1, mean(INCWAGE)],
                                  emp = sum(PERWT)),
...
plot_usmap(regions = "states", 
           data = dt.wage_growth[group == 2&ed_type=='STEM', list(fips = STATEFIP,wage_growth)]
           , values = "wage_growth", lines = "black") +
  scale_fill_continuous(low ="white", high = "red", name = "Computer Exposure (1980)") +
  theme(legend.position = "right")
# Seems to check out

dt.wage_growth <- dcast(dt.wage_growth, STATEFIP + group ~ ed_type, value.var = c('wage_growth','frac_emp'))
# Check correlations
ggplot(dt.wage_growth, aes(wage_growth_STEM,frac_emp_STEM))+geom_point()+geom_smooth(method = 'lm')
...
model <- glm(STEM ~wage_growth_STEM_std*frac_emp_STEM_std + wage_growth_state + factor(STATEFIP),
             family=binomial(link='logit'),dt.marjor_wage[DEGFIELD !=0])
summary(model)
# Subset to movers
model <- glm(STEM ~wage_growth_STEM_std*frac_emp_STEM_std + factor(STATEFIP) + wage_growth_state,
