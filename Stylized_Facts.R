...
dt.occ_wage[order(YEAR), del_w := log(shift(w,1,type = 'lead')) - log(w), by = list(OCC2010) ]
...
summary(l.model)
l.model <- lm(del_w~exp  + frac_col + frac_man + frac_trans + frac_trade + frac_public 
              , dt.cty_wage_exp, weights = dt.cty_wage_exp$emp)
summary(l.model)
xtable(l.model, digits = 3)
...
dt.college_zip_2000 <- fread("Data/College_Info/IPEDS_IC/hd2000.csv")[,list(unitid, zip= ifelse(nchar(zip)> 5, floor(as.numeric(zip)/10000),zip) , STUSAB = stabbr)]
...
dt.col_deg_2000 <- dt.ipeds_cty_2000[,list(year = 2000, comp = sum((floor(as.numeric(cip))==11)*num ), 
                                           total = sum((floor(as.numeric(cip))==99)*num)), by = list(unitid, county, STUSAB)]
dt.col_deg <- rbind(dt.col_deg_1980,dt.col_deg_2000)

dt.col_deg[order(year), del_comp := shift(comp, 1,type = 'lead') - comp, by = list(unitid, county, STUSAB)]
dt.col_deg[order(year), del_comp_share := shift(comp/total, 1,type = 'lead') - comp/total, by = list(unitid, county, STUSAB)]
...
# Add industry and fraction college controls
l.model <- lm(del_comp ~ exp + total  + frac_man + frac_trans + frac_trade + frac_public, dt.col_deg_exp )
summary(l.model)
...
dt.cty_age_exp[order(YEAR, STATEFIP, COUNTYFIP,Youth ), del_tech_pred := shift(log(tech_pred),1,type = 'lead')-log(tech_pred), by = list(STATEFIP, COUNTYFIP,Youth )]
...
dt.ind_emp[,comp_int := tech_pred > median(tech_pred), by = list(YEAR)]
dt.ind_emp[order(YEAR), del_emp := shift(log(num),1,type = 'lead') - log(num), by = list(STATEFIP, COUNTYFIP, IND1990)]
...
dt.deg_8499 <- merge(dt.deg_8499[year == 1980, list(comp_1980 = log(comp+1), total_1980 = log(total+1), county, STUSAB)],
                     dt.deg_8499[year > 1980,list(comp_int = sum(log(comp+1), na.rm = T), total_int = sum(log(total+1), na.rm = T)), by = list(county, STUSAB)], 
                     by = c('county','STUSAB'))
